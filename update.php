...
<html lang="en">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="exampleInputPassword1" placeholder="Cardno" name="date_of_purchase">
    <label>Answer In Detail Why Do You Want A Pet </label>
                        <input type="textarea" rows="8" cols="3" class="form-control" name="detail">

  </div>
  <button type="submit" class="btn btn-primary" name="submit" class="text-light">Update</button>
  <button type="submit" class="btn btn-primary" name="submit" c><a href="./view.php" lass="text-light">Go Back</button></a>
</form>
    </div>
  </body>
</html>
...
if(isset($_POST['submit']))
{
    ...
    $sql_query="UPDATE petdetails SET ID=$ID, fname='$fname', lname='$lname',email='$email',pno='$pno',date_of_purchase='$date_of_purchase',detail='$detail' WHERE ID=$ID";
